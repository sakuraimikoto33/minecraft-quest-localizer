...
from ..json5 import Json5ParseError, parse_json5_language
from ..output_guard import PathSnapshot, assert_path_unchanged, assert_source_unchanged, snapshot_path
from ..snbt import SnbtCompound, SnbtParseError, SnbtScalar, SnbtString, parse_snbt
from .base import (
    QuestAdapter,
...
@dataclass(slots=True)
class _OutputPlan:
    source_by_relative: dict[Path, _SplitDocument]
...
class FtbSplitJson5Adapter(QuestAdapter):
    id = "ftb_split_json5"
    label = "Minecraft 26.1.2 / FTB Quests 26.1.2.1以降（分割JSON5）"
    description = "lang/<locale>/ 以下の分割 JSON5 を同じ構造の対象localeへ翻訳します。"
    ...
    def probe(self, path: Path, source_locale: str) -> int:
        ...
        if source_dir:
            ...
            if source_files:
                directly_selected = (
                    path.suffix.lower() == ".json5"
                    and safe_is_regular_file(
                        path,
                        "分割 JSON5 locale の原文ファイル",
                    )
                )
                return 95 if directly_selected else 100
        return 0
    ...
    def load(
        ...
        if source_dir is None:
            raise AdapterError(f"lang/{source_locale}/ ディレクトリが見つかりません: {path}")
        ...
        directly_selected = (
            path.suffix.lower() == ".json5"
            and safe_is_regular_file(path, "分割 JSON5 locale の原文ファイル")
        )
        source_files = (
            [path]
        ...
        if not source_files:
            raise AdapterError(f"JSON5 翻訳ファイルが見つかりません: {source_dir}")

        documents: list[_SplitDocument] = []
        units: list[TranslationUnit] = []
        counter = 0
        seen_keys: dict[str, Path] = {}
        for document_index, source_file in enumerate(source_files):
            text, _encoding, newline = read_text_detect(source_file)
            try:
                parsed = parse_json5_language(text)
            except Json5ParseError as exc:
                raise AdapterError(f"JSON5 翻訳ファイルを解析できません: {source_file} ({exc})") from exc
            ...
            if duplicate:
                raise AdapterError(
                    f"JSON5 翻訳ファイル間でキーが重複しています: {duplicate}\n"
                    ...
                    for item_index, item in enumerate(value):
                        ...
                        units.append(
                            TranslationUnit(
                                ...
                                source=item,
                                context=f"FTB Quests translation list {key}, line {item_index + 1}",
                                source_path=str(source_file),
                                ordinal=len(units),
                                category=classify_ftb_text(key),
                            )
        ...
        if not units:
            raise AdapterError(f"JSON5 翻訳ファイルに翻訳文字列がありません: {source_dir}")
        reject_nested_reparse_points(source_dir, "分割 locale の原文")
        ...
        if actual_source_locale.lower() != source_locale.lower():
            warnings.append(
                f"data.json5 の fallback_locale に従い、原文localeを {actual_source_locale} として読み込みました。"
            )
    ...
    def write(
        ...
        for relative_path, target in zip(plan.relative_paths, plan.targets):
            ...
            if document is not None:
                for key in document.values:
                    ...
                    if isinstance(unit_id, str):
                        ...
                        rendered_values[key] = [
                            translations[item_id] for item_id in unit_id
                        ]
            ...
            # Strict JSON is valid JSON5 and avoids changing any quest-data file.
            rendered = json.dumps(rendered_values, ensure_ascii=False, indent=2) + "\n"
            try:
                verified = parse_json5_language(rendered)
            except Json5ParseError as exc:
                raise AdapterError(f"生成した分割 locale JSON5 の自己検証に失敗しました: {exc}") from exc
            if list(verified) != list(rendered_values) or any(
                ...
                raise AdapterError(
                    f"生成した分割 locale JSON5 のキーまたは値型が選択結果と一致しません: "
                    f"{relative_path}"
                )
            newline = (
                document.newline
...
def _locate_source_dir(path: Path, source_locale: str, allow_fallback: bool) -> tuple[Path | None, str]:
    ...
    if allow_fallback and quest_root:
        fallback = _read_fallback_locale(quest_root / "data.json5", quest_root)
...
def _load_existing(target_dir: Path, documents: list[_SplitDocument]) -> dict[str, str]:
...
def _existing_from_documents(
    ...
    for target_document in target_documents.values():
        for key, target_value in target_document.values.items():
            unit_id = by_key.get(key)
            if isinstance(unit_id, str) and isinstance(target_value, str):
                existing[unit_id] = target_value
            elif isinstance(unit_id, list) and isinstance(target_value, list):
                for item_id, value in zip(unit_id, target_value):
                    existing[item_id] = value
    return existing
...
def _read_target_documents(
    ...
    for target in find_regular_files_by_suffix_no_reparse(
        ...
        try:
            ...
            values = OrderedDict(parse_json5_language(text).items())
        except (OSError, UnicodeError, Json5ParseError) as exc:
            raise AdapterError(f"既存の翻訳先 locale JSON5 を解析できません: {target} ({exc})") from exc
        duplicate = next((key for key in values if key in seen), None)
        if duplicate:
            raise AdapterError(
                f"既存の翻訳先 locale JSON5 間でキーが重複しています: {duplicate}\n"
                f"- {seen[duplicate]}\n- {target}"
            )

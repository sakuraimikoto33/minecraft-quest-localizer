...
モデル一覧には、翻訳で必須のResponses APIとStructured Outputs（`json_schema`）を利用できないモデルを表示しません。埋め込み、モデレーション、音声、画像、リアルタイム、検索など明らかに別用途のモデルは送信先に関係なく除外し、APIが対応endpointやStructured Outputs非対応を明示した場合も除外します。OpenAI公式APIでは、既知の旧世代・旧スナップショットも除外します。互換APIの未知のモデルIDは、互換サービス側に対応情報がない場合があるため、明らかな非翻訳モデルでなければ直接入力できます。翻訳開始時にも同じ判定を行います。

...
残量不足時は、次のリクエストを送信せず、完全に検証済みのTranslationUnitだけを表示します。「途中まで書き込む」を選んだ場合だけ `adapter.write(..., selected_unit_ids=...)` で部分出力し、「書き込まず終了」またはダイアログを閉じた場合は既存ファイルを変更しません。複数行配列・JSONコンポーネント・表示上の固有名詞グループは全体が揃わない限り部分出力へ含めません。通常のAPIエラー、応答破損、保護検証失敗、入力変更では部分保存を提案しません。
